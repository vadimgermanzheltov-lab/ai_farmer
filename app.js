...
async function boot() {
  try {
    ...
    console.error("Smart Poultry House bootstrap failed", error);
    showStartupFallback();
  }
}
...
function applyEmbeddedDefaults() {
  state.data = {
    appConfig: EMBEDDED_DATA.appConfig,
    scenarios: EMBEDDED_DATA.scenarios,
    poultryCatalog: EMBEDDED_DATA.poultryCatalog,
    housingNorms: EMBEDDED_DATA.housingNorms,
    recommendationTemplates: EMBEDDED_DATA.recommendationTemplates,
    safetyRules: EMBEDDED_DATA.safetyRules,
    visualAssets: EMBEDDED_DATA.visualAssets
  };
  state.env = {};
  state.isBusy = false;
...
function showStartupFallback() {
  ...
  appRoot.innerHTML = `
    <div class="app-shell">
      <section class="chat-shell" style="grid-column: 1 / -1; min-height: auto;">
        <div class="chat-topbar">
          <div class="chat-title">
            <strong>Умный птичник</strong>
            <span>Интерфейс запустился с ошибкой, но сайт доступен</span>
          </div>
          <button class="reset-button" type="button" onclick="window.location.reload()">Обновить страницу</button>
        </div>
        <div class="messages">
          <article class="message assistant">
            <div class="message-label">Подсказка</div>
            <p>Не удалось полностью загрузить интерфейс. Попробуйте обновить страницу. Если ситуация повторится, значит нужно поправить стартовый скрипт на опубликованной версии.</p>
          </article>
        </div>
      </section>
    </div>
